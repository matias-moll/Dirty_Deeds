...
namespace Dal
{
    public static class ConsultasDinamicas
    {
                                                          ...
                                                          string whereRubros, string whereDescripcion)
        {
            ...
            string fromYSubquery = " FROM ( SELECT  Publicacion.Codigo, Publicacion.Presentacion, Publicacion.Stock,  Visibilidad.Precio,Publicacion.Fecha,DIRTYDEEDS.Visibilidad.Descripcion as Visibilidad, Publicacion.FechaVto, Publicacion.Tipo, Publicacion.AceptaPreguntas, ROW_NUMBER() OVER (ORDER BY Visibilidad.Precio) AS numero_row, Usuario.Id as Id_Usuario, Usuario.Discriminante FROM  DIRTYDEEDS.Publicacion join DIRTYDEEDS.Visibilidad on Publicacion.IdVisibilidad = Visibilidad.Id join DIRTYDEEDS.Estado on Publicacion.IdEstado = Estado.Id join DIRTYDEEDS.Usuario on Usuario.Id = Publicacion.IdUsuario ";
